...
import math
from command import moveBootThreeTCs
# import pyautogui
...
class Player():
    ...
    def canMoveRoad(self, p):   # Checking the legality of the road movement on the chosen path.
        ...
        if p.transportCounter is None:  # If there is *no* transport counter set on the road, we return False.
            ...
            # elif num > avb:     # If the number of cards required is more than the available amount.
            #     alternate = input(
            #         "Choose to use 3 travel cards: Type in \"yes\" or \"pass\": ")  # We prompt player to choose if they want to select 3 travel cards or pass.
            #     if alternate == "yes":
            #         if len(self.travel_cards) >= 3:
            #             return True, 500, " "   # We return the legality as true and a number that can't be evaluated but used as a flag.
            #         else:
            #             print("Not enough travel cards left in deck.")
            #             return False, -1, " "  # We return the move as illegal.
            #     else:
            #         return False, -1, " "  # We return the move as illegal.
            else:
    ...
    def move(self, x, y, game):
        ...
        alternate = False # A flag to check if a player is prompted to check three travel cards.
        if number > 1:      # If there's more than 1 option
            # ground = self.promptPathSelection()
            # ground = input("Select Travel route: Type in \"water\" or \"land\": ") # We prompt player to choose an option.
            # pyautogui.alert("Click on either the Land or Water button.")
            ...
            return True, 2
            # self.destinationTown = dest
            # chosenLand = self.clickedLand()
            # chosenWater = self.clickedWater()
            # if chosenLand and self.town.isAdjacent(dest):     # If the user wants to use the land route we check the legality of the move.
            #     print("Figuring out if it can move on land:")
            #     moveOnGround, num, nam = self.canMoveRoad(road_options[0]) # Checking the legality of the land movement.
                # if num == 500:  # Command has changed to prompt the player to pick 3 travel cards to make the move.
                #     self.command = moveBootThreeTCs(self)   # Change the player's command to moveBootThreeTc's to be executed by server
                #     self.dest = dest
                #     return True  # return moved/about to move as True
            # elif chosenWater and self.town.isAdjacent(dest):
            #     print("Figuring out if it can move on water:")
            #     moveOnWater, num = self.canMoveWater(water_options[0])
