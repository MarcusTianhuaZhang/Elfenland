...
def main(username,variant,numPlayers,host,game_id,saveID):
    if saveID != "":
        #IS A SAVED GAME -> load saved game
        pass
    run = True
    n = Network()
    FPS = 80
    ...
    def game_menu(GAME,p):
        ...
        while True:
            m = n.send([GAME, p])
            GAME = m[0]
            allColours = GAME.avail_colours
            startText = font.render("Color Select", True, white)
            blueText = font.render("Blue", True, blackish)
            redText = font.render("Red", True, blackish)
            blackText = font.render("Black", True, white)
            ...
            screen.blit(startText, ((screen_width - startText.get_width()) / 2, 100))

            if "blue" in allColours:
                BlueButton = create_button((screen_width / 2) - 100, int(screen_height * .33), 200, 50, b2, b1)
                screen.blit(blueText, ((screen_width / 2) - (blueText.get_width() / 2), int(screen_height * .33)))
                if BlueButton:
                    return ("blue")

            if "red" in allColours:
                RedButton = create_button((screen_width / 2) - 100, screen_height / 2, 200, 50, r2, r1)
                screen.blit(redText, ((screen_width / 2) - (blueText.get_width() / 2), screen_height / 2))
                if RedButton:
                    return ("red")

            if "black" in allColours:
                BlackButton = create_button((screen_width / 4) - 100, int(screen_height * .33), 200, 50, bl2, bl1)
                screen.blit(blackText, ((screen_width / 4) - (blackText.get_width() / 2), int(screen_height * .33)))
                if BlackButton:
                    return ("black")

            if "purple" in allColours:
                purpleButton = create_button((screen_width / 4) - 100, int(screen_height / 2), 200, 50, p2, p1)
                screen.blit(purpleText, ((screen_width / 4) - (purpleText.get_width() / 2), screen_height / 2))
                if purpleButton:
                    return ("purple")

            if "green" in allColours:
                GreenButton = create_button((screen_width / (4 / 3)) - 100, int(screen_height * .33), 200, 50, g2, g1)
                screen.blit(greenText, ((screen_width / (4 / 3)) - (greenText.get_width() / 2), int(screen_height * .33)))
                if GreenButton:
                    return ("green")

            if "yellow" in allColours:
                yellowButton = create_button((screen_width / (4 / 3)) - 100, int(screen_height / 2), 200, 50, y2, y1)
                screen.blit(yellowText, ((screen_width / (4 / 3)) - (yellowText.get_width() / 2), screen_height / 2))
                if yellowButton:
                    return ("yellow")
            ...
            if event.type == pygame.KEYDOWN and p.name == host:
                if event.key == pygame.K_s:
                    ...
                    USER_INP = simpledialog.askstring(title="ID",
                                                    prompt="What would you like the Game ID to be?")
    ...
    while run:
        ...
        if (m!= None):
            ...
            p.clicked = CLICK
            if not p.ready:
                p.colour = game_menu(GAME,p)
                p.command = changeColourCommand(p, p.colour, variant,numPlayers,username)
